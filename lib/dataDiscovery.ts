...
function calculateQualityScores(
  ...
  qualityIssues: QualityIssue[],
  businessRules: BusinessRule[]
): Omit<DataDiscoveryResult, 'dataPatterns' | 'qualityIssues' | 'businessRules' | 'recommendations'> {
  const totalAnomalies = qualityIssues.length;
  const completenessScore = Math.max(50, 100 - totalAnomalies * 2);
  const consistencyScore = Math.max(50, 100 - (businessRules.filter(r => r.complianceScore < 80).length * 3));
  const uniquenessScore = Math.max(50, 100 - (qualityIssues.filter(i => i.issueType === 'DUPLICATE').length * 5));
  const accuracyScore = Math.max(50, 100 - (qualityIssues.filter(i => i.issueType === 'INVALID_FORMAT').length * 3));
  const dataQualityScore = Math.round((completenessScore + consistencyScore + uniquenessScore + accuracyScore) / 4);
...
function generateDataQualityRecommendations(qualityIssues: QualityIssue[], businessRules: BusinessRule[]): string[] {
  const recs: string[] = [];
  if (qualityIssues.some(i => i.issueType === 'NULL_VALUE')) recs.push('Rever colunas com alta taxa de valores nulos');
  if (qualityIssues.some(i => i.issueType === 'DUPLICATE')) recs.push('Implementar validações para evitar duplicatas');
  if (qualityIssues.some(i => i.issueType === 'INVALID_FORMAT')) recs.push('Validar formatos de documentos (CPF/CNPJ) e emails');
  if (qualityIssues.some(i => i.issueType === 'OUTLIER')) recs.push('Checar outliers em colunas numéricas');
  if (businessRules.some(r => r.complianceScore < 80)) recs.push('Ajustar regras de negócio com baixa conformidade');
  if (recs.length === 0) recs.push('Nenhuma recomendação crítica. Manter monitoramento contínuo');
  return recs;
}
...
async function analyzeNullValues(_pool: sql.ConnectionPool): Promise<QualityIssue[]> {
  return [];
}
